...
<a name="getShopPaymentAccountLedgerEntry"></a>
...
public class Example {
  public static void main(String[] args) {
    ...
    LedgerEntryApi apiInstance = new LedgerEntryApi(defaultClient);
    Long shopId = 56L; // Long | The unique positive non-zero numeric ID for an Etsy Shop.
    Long ledgerEntryId = 56L; // Long | The unique ID of the shop owner ledger entry.
    try {
      PaymentAccountLedgerEntry result = apiInstance.getShopPaymentAccountLedgerEntry(shopId, ledgerEntryId);
      System.out.println(result);
    } catch (ApiException e) {
      System.err.println("Exception when calling LedgerEntryApi#getShopPaymentAccountLedgerEntry");
      System.err.println("Status code: " + e.getCode());
      System.err.println("Reason: " + e.getResponseBody());
      System.err.println("Response headers: " + e.getResponseHeaders());
      e.printStackTrace();
...
|------------- | ------------- | ------------- | -------------|
| **shopId** | **Long**| The unique positive non-zero numeric ID for an Etsy Shop. | |
| **ledgerEntryId** | **Long**| The unique ID of the shop owner ledger entry. | |
